...
$.extend(InlineEditor.prototype, {
	...
	handleCancelEditor: function(anEvent) {
		...
		if (hasContent(enteredText) 
			...
			this.restoreOriginalValue();   //cambiado para que al cancelar devuelva el valor anterior by raul this.originalValue
		this.reinit();
	},
	///////////////////////////////////////////////////
	handleSaveEditor: function(anEvent) {
		...
		if (this.didForgetRequiredText(enteredText)) {
			this.handleCancelEditor(anEvent);
			this.reportError("Debes introducir un dato para poder guardarlo");
			return;
		}
	...
	handleSubmitToCallback: function(enteredText) {
		...
		if (this.settings.callback_skip_dom_reset)
			...
			this.reportError("Error: No se ha guardado el valor: " + enteredText);
			this.restoreOriginalValue();
		}
		else
		...
		$.ajax({
			...
			error: function(request) {
				...
				if (that.settings.error)
					...
					that.reportError("Error: " + request.responseText || 'Unspecified Error');
			}
		});
	},
